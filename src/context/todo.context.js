...
const todoReducer = (todos = initialTodos, action) => {
	switch (action.type) {
		case ADD_TODO:
			return [
				...todos,
...
const addTodo = (dispatch) => (newTask) => {
	dispatch({ type: ADD_TODO, newTask });
};

const editTodo = (dispatch) => (id, newTask) => {
...
export const { Context, Provider } = createDataContext(todoReducer, { addTodo, editTodo, removeTodo, toggleTodo }, []);
